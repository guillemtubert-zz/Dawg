...
matchRouter.post("/:id", (req, res, next) => {
  ...
  Dog.findById(req.session.currentUser._id)
    ...
    .then((dog) => {
      ...
      if (dog.interactions.length) {
        for (let i = 0; i < dog.interactions.length; i++) {
          ...
          if (
            String(interaction.dogOneId) == matchedDog ||
            String(interaction.dogTwoId) == matchedDog
          ) {
      ...
      if (!alreadyMatched) {
        ...
        Match.create({ dogOneId, dogTwoId, dogOneAnswer })
          .then((createMatch) => {
            ...
            const pr1 = Dog.findByIdAndUpdate(req.session.currentUser._id, {
              $push: { interactions: createMatch },
            });
            const pr2 = Dog.findByIdAndUpdate(req.session.currentUser._id, {
              $push: { selected: dogTwoId },
            });

            const pr3 = Dog.findByIdAndUpdate(matchedDog, {
              $push: { interactions: createMatch },
            });
        ...
        Match.findByIdAndUpdate(
          alreadyMatched._id,
          { dogTwoAnswer, success: state },
          { new: true }
        )
          .then((updatedMatch) => {
            const { dogOneId } = updatedMatch;
            const pr = Dog.findByIdAndUpdate(req.session.currentUser._id, {
              $push: { selected: dogOneId },
            });
            return pr;
          })
          .then(() => {
